...
class SmartNotificationProvider extends ChangeNotifier {
  ...
  final WeatherNotificationService _weatherService =
  ...
  List<SmartNotification> get recentNotifications => _notificationsByTrip.values
      .expand((list) => list)
      .where((n) => !n.isRead)
      .take(3)
      .toList();
  ...
  Future<void> initialize(String tripId) async {
    ...
    try {
      ...
      _periodicTimer ??= Timer.periodic(const Duration(minutes: 5), (timer) {
        ...
        for (String tripId in _initializedTrips.keys.where(
          (key) => _initializedTrips[key] == true,
        )) {
  ...
  Future<void> clearTestNotifications(String tripId) async {
    try {
      if (_notificationsByTrip[tripId] != null) {
        final testNotifications = _notificationsByTrip[tripId]!
            .where((n) => n.id.startsWith('test_'))
            .toList();
    ...
    try {
      ...
      try {
        final reminders = await _reminderService
            .checkUpcomingActivities(tripId)
            .timeout(
              const Duration(seconds: 10),
              onTimeout: () {
      ...
      try {
        final budgetWarnings = await _budgetService
            .checkTripBudgetStatus(tripId)
            .timeout(
              const Duration(seconds: 10),
              onTimeout: () {
  ...
  Future<void> checkBudgetOnActivity(
    String tripId,
    String activityId,
    double actualCost,
  ) async {
    try {
      final warning = await _budgetService.checkBudgetOverage(
        activityId,
        actualCost,
        tripId: tripId,
      );
  ...
  Future<void> handleActivityCheckIn(
    String tripId,
    String activityId,
  ...
  @Deprecated(
    'Use handleExpenseCreatedWithResponse with full expense response instead',
  )
  ...
  Future<void> _addWeatherNotification(
    WeatherAlert alert,
    String tripId,
  ) async {
    final notification = SmartNotification(
      ...
      title: 'Weather Alert',
      message:
          '${alert.description} at ${alert.location}. Temperature: ${alert.temperature.round()}°C',
      createdAt: DateTime.now(),
    ...
    final existingNotification = _notificationsByTrip[tripId]!
        .where(
          (n) =>
  ...
  Future<void> _addBudgetWarningNotification(
    ...
    final notification = SmartNotification(
      ...
      type: NotificationType.budget,
      severity: warning.overagePercentage > 50
          ? NotificationSeverity.critical
          : NotificationSeverity.warning,
      title: 'Budget Alert',
      message:
          '${warning.activityTitle} has exceeded ${warning.overagePercentage.round()}% of planned budget',
      createdAt: DateTime.now(),
      icon: Icons.warning,
      color: Colors.orange,
      data: warning.toJson(),
  ...
  Future<void> _addActivityReminderNotification(
    ...
    final notification = SmartNotification(
      ...
      message:
          '${reminder.activityTitle} will start in ${reminder.minutesUntilStart} minutes at ${reminder.location}',
      createdAt: DateTime.now(),
      icon: Icons.schedule,
      color: Colors.green,
    ...
    final existingReminder = _notificationsByTrip[tripId]!
        .where(
          (n) =>
  ...
  Future<void> markAsRead(String notificationId, String tripId) async {
    if (_notificationsByTrip[tripId] != null) {
      final index = _notificationsByTrip[tripId]!.indexWhere(
        (n) => n.id == notificationId,
      );
      if (index != -1) {
        _notificationsByTrip[tripId]![index] =
            _notificationsByTrip[tripId]![index].copyWith(isRead: true);
    ...
    if (_notificationsByTrip[tripId] != null) {
      for (int i = 0; i < _notificationsByTrip[tripId]!.length; i++) {
        if (!_notificationsByTrip[tripId]![i].isRead) {
          _notificationsByTrip[tripId]![i] = _notificationsByTrip[tripId]![i]
