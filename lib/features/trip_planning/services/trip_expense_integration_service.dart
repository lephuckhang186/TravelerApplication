...
class TripExpenseIntegrationService {
  ...
  void _ensureExpenseProviderAuth() {
    try {
      ...
      if (user != null && _expenseProvider != null) {
        ...
        debugPrint('DEBUG: Integration - Ensuring expense provider has auth for user: ${user.uid}');
      } else {
  ...
  Future<bool> syncActivityExpense(ActivityModel activity) async {
    if (_expenseProvider == null || activity.budget == null) {
      debugPrint('DEBUG: Integration - No expense provider or budget available');
      return false;
    }

    // Use actual cost if available, otherwise use estimated cost
    final amount = activity.budget!.actualCost ?? activity.budget!.estimatedCost;
    ...
    try {
      debugPrint('DEBUG: Integration - Attempting to create expense: amount=$amount, type=${activity.activityType.value}');
      debugPrint('DEBUG: Integration - ExpenseProvider available: ${_expenseProvider != null}');
      ...
      if (success && activity.id != null) {
        ...
        if (activityId != null) {
          ...
          _activityExpenseMap[activityId]!.add(DateTime.now().millisecondsSinceEpoch.toString());
        }
        ...
        if (tripId != null) {
          ...
          _tripExpenseMap[tripId]!.add(DateTime.now().millisecondsSinceEpoch.toString());
        }
      }
  ...
  Future<Map<ExpenseCategory, double>> getTripExpensesByCategory(String tripId) async {
  ...
  Future<TripBudgetSummary> getTripBudgetSummary(TripModel trip) async {
    ...
    return TripBudgetSummary(
      ...
      percentageUsed: totalBudget > 0 ? (totalExpenses / totalBudget * 100) : 0.0,
  ...
  List<CalendarActivityInfo> getActivitiesForCalendar(List<TripModel> trips) {
    ...
    for (final trip in trips) {
      for (final activity in trip.activities) {
        ...
        activities.add(CalendarActivityInfo(
          id: activity.id ?? '',
  ...
  Future<TripStatistics> getTripStatistics(List<TripModel> trips) async {
    ...
    for (final trip in trips) {
      ...
      for (final entry in budgetSummary.expensesByCategory.entries) {
        categorySpending[entry.key] = (categorySpending[entry.key] ?? 0.0) + entry.value;
      }
    }
    ...
    return TripStatistics(
      ...
      averageSpendingPerTrip: trips.isNotEmpty ? totalSpent / trips.length : 0.0,
      averageSpendingPerActivity: totalActivities > 0 ? totalSpent / totalActivities : 0.0,
    );
  }
